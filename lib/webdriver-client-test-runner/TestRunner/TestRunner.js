...
(function (TestRunner) {
    ...
    var TestRunnerInternal = (function () {
        ...
        TestRunnerInternal.prototype.run = function () {
            var _this = this;
            return externals_1.Q()
                .then(function () { return exports_1.HttpLocalFileServer.start(); })
                .then(function () {
                ...
                .finally(function () { return exports_1.HttpLocalFileServer.stop(); });
        };
        ...
        TestRunnerInternal.prototype.initJasmine = function () {
            ...
            jasmine.addClientLinks = function (addAdditionalLinks, relativeToCallerFilePath) {
                var files = _this.config.execFiles ? exports_1.Helpers.getFilesByGlob(_this.config.execFiles, _this.config.rootDir) : [];
                return _this.addFileLinksOnTestPage(_this.config.files, _this.config.rootDir).then(function () {
                    if (addAdditionalLinks)
                        return;
                    return _this.addFileLinksOnTestPage(addAdditionalLinks, relativeToCallerFilePath
                        ? externals_1.Path.dirname(exports_1.Helpers.getCallerFilePath())
                        : _this.config.rootDir);
                });
            };
            jasmine.execClientScripts = function (execAdditionalFiles, relativeToCallerFilePath) {
                var files = _this.config.execFiles ? exports_1.Helpers.getFilesByGlob(_this.config.execFiles, _this.config.rootDir) : [];
                if (execAdditionalFiles) {
                    var rootDir = relativeToCallerFilePath ? externals_1.Path.dirname(exports_1.Helpers.getCallerFilePath()) : _this.config.rootDir;
                    files = files.concat(exports_1.Helpers.getFilesByGlob(execAdditionalFiles, rootDir));
                }
        ...
        TestRunnerInternal.prototype.initTestPage = function (url) {
            ...
            return externals_1.Q.fcall(function () {
                ...
                .url(externals_1.FS.existsSync(url) ? exports_1.HttpLocalFileServer.getUrl(url) : url)
                .initConsoleLogReader()
                .then(function () { return jasmine.addClientLinks(); }) // adds css/script links.
                .then(function () { return jasmine.execClientScripts(); }) // executes scripts.
                .then(function () {
        ...
        TestRunnerInternal.prototype.addFileLinksOnTestPage = function (glob, rootDir) {
            var _this = this;
            if (!glob) {
                return this.webdriverClient;
            }
            return this.webdriverClient.getUrl().then(function (url) {
                var isLocalFile = exports_1.HttpLocalFileServer.isHttpLocalFileServerUrl(url);
                var convertedFiles = externals_1._.flatten((externals_1._.isArray(glob) ? glob : [glob]).map(function (v) {
                    if (externals_1.Url.parse(v) && externals_1.Url.parse(v).host) {
                        return v;
                    }
                    else {
                        if (isLocalFile) {
                            return exports_1.Helpers.getFilesByGlob(v, rootDir).map(function (x) { return exports_1.HttpLocalFileServer.getUrl(x); });
                        }
                    }
                })).filter(function (x) { return !!x; });
                return convertedFiles;
            }).then(function (files) { return _this.webdriverClient.executeAsync(function (files, done) {
